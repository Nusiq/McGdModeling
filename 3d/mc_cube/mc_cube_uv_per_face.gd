...
func load_from_object(obj: Dictionary, path_so_far: Array=[]) -> void:
    ...
    var side_names: Array[String] = ["up", "down", "north", "south", "east", "west"]
    ...
    for i in range(side_names.size()):
        ...
        var side_val := XJSON.get_object_from_object(obj, side_name, path_so_far)
        if side_val.error:
            Logging.warning(str(side_val.error.pass_()))
            continue
        var side_path := path_so_far + [side_name]
        # UV
        var uv_val := XJSON.get_vector2_from_object(side_val.value, "uv", side_path)
        if uv_val.error:
